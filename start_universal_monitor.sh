...
# إنشاء مجلد العامل
mkdir -p ~/universal_monitoring

# إنشاء ملف المراقبة الشامل
cat > universal_monitor.py << 'EOF'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
        ...
        def discovery_loop():
            while self.running:
                try:
                    ...
                    for device_ip in devices:
                        # التحقق من عدم وجود خيط لهذا الجهاز
                        device_threads = [t for t in threading.enumerate() if hasattr(t, 'device_ip') and t.device_ip == device_ip]
...
# تشغيل النظام
echo "🚀 بدء تشغيل نظام المراقبة الشامل..."
